...
class Cluster:
    ...
    def __init__(self, label=None, learning_rate=0.005):
    ...
    def update_pattern_confidence(self):
        ...
        for phrase in self.phrases:
            #print (phrase.relations)
            # print("Phrase", phrase)
            sentence = Sentence(phrase.full_sentence)
            relations = phrase.relations
            #print ('phrase relation',relations)
            found_relations = self.get_relations(phrase.tagged_tokens)
            #print ('get_relation',found_relations)
    ...
    def get_relations(self, tokens):
            ...
            relations = []
            entity_type_indexes = {}
            #print (self.pattern.parse_expression.scan(tokens))
            mylist = [t[0] for t in tokens]
            for pattern_relation in self.pattern.relations:
                count = 0
                for pattern_entity in pattern_relation.entities:
                    l = ['-', '–', '−', '~', '∼', '˜']
                    for i in l:
                        if i in pattern_entity.text:
                            try:
                                if mylist[pattern_entity.start] == pattern_entity.text.split(i)[0] and mylist[
                                    pattern_entity.start:pattern_entity.end] == [pattern_entity.text.split(i)[0],i,pattern_entity.text.split(i)[1]]:
                                    count += 1
                            except:
                                pass
                    if not any(x in pattern_entity.text for x in l):
                        #print (Sentence(self.pattern.to_string()).tagged_tokens)
                        #print (pattern_entity.text.split(' ')[0],pattern_entity.start,pattern_entity.end,mylist)
                        try:
                            if mylist[pattern_entity.start] == pattern_entity.text.split(' ')[0] and mylist[pattern_entity.start:pattern_entity.end] == pattern_entity.text.split(' '):
                                count += 1
                        except:
                            pass

                if count == len(pattern_relation):
                    relations.append(pattern_relation)
            return relations
    ...
    #     #     #     for pattern_relation in self.pattern.relations:
    #     #     #         print (pattern_relation)
    #     #     #         found_entities = []
    #     #     #         for pattern_entity in pattern_relation.entities:
    #     #     #             print (type(pattern_entity.text))
    #     #     #             idx_start = pattern_entity.start
    #     #     #             idx_end = pattern_entity.end
    #     #     #             #print([t[0] for t in tokens][idx_start:idx_end])
    ...
    #     #     #             for i in l:
    #     #     #                 if i in entity_text:
    #     #     #                     entity_tokens = [entity_text.split(i)[0],i,entity_text.split(i)[1]]
    #     #     #                     for i in range(len(mylist)):
    #     #     #                         if mylist[idx_start] == entity_tokens[0] and mylist[idx_start:idx_end] == entity_tokens:
    #     #     #                             start_idx = idx_start
    #     #     #                             end_idx = idx_end
    #     #     #                         else:
    #     #     #                             start_idx = None
    ...
    #                 entity_tokens = entity_text.split(' ')
    #                 start_idx, end_idx = subfinder([t[0] for t in tokens], entity_tokens)
    #                 found_entity = Entity(entity_text, pattern_entity.tag, pattern_entity.parse_expression, start_idx,
    #                                       end_idx)
    #                 found_entities.append(found_entity)
    #             found_relation = Relation(found_entities, confidence=1)
    #             relations.append(found_relation)
    #
    #     return relations