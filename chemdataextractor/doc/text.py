...
from chemdataextractor.model.parsertemplate import PropertyParserTemplate
from chemdataextractor.model.parsertemplate import MultiQuantityModelTemplateParser
...
class Sentence(BaseText):
    ...
    def records(self):
        ...
        tagged_tokens = [(CONTROL_RE.sub('', token), tag) for token, tag in self.tagged_tokens]
        tokens = [i[0] for i in tagged_tokens]
        pos = [i[1] for i in tagged_tokens]
        if 'Fig.' in tokens and 'CD' in pos:
            distance = abs(tokens.index('Fig.') - pos.index('CD'))
        else:
            distance = 100
        ...
        for model in self._streamlined_models:
            for parser in model.parsers:
                #print (type(parser))
                if hasattr(parser, 'parse_sentence') and not isinstance(parser,AutoSentenceParser) and distance >= 3 and ('%', 'NN') not in tagged_tokens:
                    try:
