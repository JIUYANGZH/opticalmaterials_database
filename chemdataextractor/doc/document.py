    ...
    def records(self):
        ...
        for i, el in enumerate(self.elements):
            ...
            for record in el_records:
                ...
                # new_record = ModelList()
                # for i in record[:-1]:
                #     new_record.insert(-1,i)
                #print(record,type(record))
                #print (ModelList(record[:-1]),type(ModelList(record[:-1])))
                #print(new_record,type(new_record))
                # if not all(isinstance(i,str) for i in new_record):
                #     if new_record not in records:
                #         log.debug(new_record.serialize())
