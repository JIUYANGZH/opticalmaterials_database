...
def value_element(units=(OneOrMore(T('NN')) | OneOrMore(T('NNP')) | OneOrMore(T('NNPS')) | OneOrMore(T('NNS')))('raw_units').add_action(merge)):
    ...
    value_scientific_notation = ( R('^\d+(\.\d+)?(x|\*|×|·|\u00B7|\u22C5)?(10|E)[\--–−~∼˜]\d+$')).add_action(merge)
    value = Optional(lbrct).hide() + (value_range|value_single|value_scientific_notation)('raw_value') + Optional(rbrct).hide()
    return value  + units
...
def value_element_plain():
    ...
    value_range = (Optional(R('^[\-–−-]$')) + (plusminus_range | joined_range | spaced_range | to_range | between_range))('raw_value').add_action(merge)
    value_single = (Optional(R('^[~∼˜\<\>]$')) + Optional(R('^[\-–−-]$')) + number)('raw_value').add_action(merge)
    value_single1 = (Optional(R('^[~∼˜\<\>]$')) + Optional(R('^(?:[\--–−~∼˜∼])$')) + number)('raw_value').add_action(join)
    #value_single2 = (Optional(R('^[~∼˜\<\>]$')) + Optional(R('^[\-–−~∼˜∼]$')) + number)('raw_value')
    value_scientific_notation = (R('^\d+(\.\d+)?(x|\*|×|×|∗|·|\u00B7|\u22C5)?(10|E)[\---–−~∼˜]?\d+$')).add_action(merge)
    value =  Optional(lbrct).hide() +  Not(I('Fig.')) + (value_range| value_single1|value_single|value_scientific_notation)('raw_value') + Not(I('wt%')|I('vol%')|I('mol%')|I('K')|I('times')|I('GPa')|I('wt')|I('vol')|I('%')|I('nm')|I('zF')|W('°')|W('KV')|W('kV')|W('MV')|I('kHz')|I('Hz')|I('GHz')|W('V')|W('J')|W('eV')|I('MHz')) + Optional(rbrct).hide() # + (R("([.,;':)])")|(R("([\s][a-z]+)"))|W(' '))
    return value
...
def extract_value(string):
    ...
    multiplier = re.findall('\*|x|×|·|⋅|E',string)
    connection = re.findall('-|-|–|−|~|∼|˜|∼',string)
    if multiplier and connection:
        values = [float(string.split(multiplier[0])[0]) * 10 ** -float((string.split(multiplier[0])[1]).split(connection[0])[1])]
    return values
...
def _find_powers(units_list):
    ...
    while i in range(len(units_list)):
        ...
        if found_power is not None and '/' not in found_power.group(0).split()[0]:
            #print (found_power.group(0).split())
