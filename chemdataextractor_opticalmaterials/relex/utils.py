...
def match_score(pi, pj, prefix_weight=0.1, middle_weight=0.8, suffix_weight=0.1):
    ...
    #sim = (prefix_weight * prefix_dot_prod) + ((middle_weight / (number_of_middles - 1))* (sum(middle_dot_prod) - 1)) + (suffix_weight * suffix_dot_prod)
...
def subfinder(mylist, entity):
    for i in range(len(mylist)):
        if mylist[i] == entity[0] and mylist[i:i+len(entity)] == entity:
            return (i, i+len(entity))
    return None, None