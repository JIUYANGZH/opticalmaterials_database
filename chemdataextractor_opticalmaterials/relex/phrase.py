...
class Phrase(object):
    ...
    def __init__(self, sentence_tokens,tagged_tokens, relations, prefix_length, suffix_length):
        """Phrase Object
    ...
    def create(self):
        ...
        prefix_tokens = [t[0] for t in
                         self.tagged_tokens[sorted_entity_list[0].start - self.prefix_length:sorted_entity_list[0].start]]
        prefix_pos = [t[1] for t in self.tagged_tokens[
        ...
        self.elements['prefix'] = {'tokens': prefix_tokens, 'pos': prefix_pos}
        ...
        for m in range(0, number_of_middles):
            ...
            middle_tokens = [t[0] for t in self.tagged_tokens[prev_entity_end:next_entitiy_start]]
            middle_pos = [t[1] for t in self.tagged_tokens[prev_entity_end:next_entitiy_start]]
            if len(middle_tokens) == 0:
                middle_tokens = ['<Blank>']
            #self.elements['middle_' + str(m+1)] = {'tokens': middle_tokens}
            self.elements['middle_' + str(m+1)] = {'tokens': middle_tokens, 'pos': middle_pos}

        suffix_tokens = [t[0] for t in self.tagged_tokens[sorted_entity_list[-1].end : sorted_entity_list[-1].end + self.suffix_length]]
        suffix_pos = [t[1] for t in
                      self.tagged_tokens[sorted_entity_list[-1].end:sorted_entity_list[-1].end + self.suffix_length]]
        if len(suffix_tokens) == 0:
            suffix_tokens = ['<Blank>']
        #self.elements['suffix'] = {'tokens': suffix_tokens}
        self.elements['suffix'] = {'tokens': suffix_tokens, 'pos': suffix_pos}
